...
export const GuessBox: React.FC<GuessBoxProps> = ({
  ...
  return (
    <button
      className={`relative w-full aspect-square hover:bg-slate-600 ${
        image !== "" && !give_up ? "bg-slate-200" : "bg-slate-700"
    ...
    >
      ...
      {image === "" && give_up && (
        <>
          <img
            src={
              collections.find((collection) => collection.id === top_guess)
                ?.image || ""
            }
            alt=""
            className={`w-full h-full  object-cover ${
          ...
          />
          <div
            className={`absolute top-0 right-0 grid place-content-center w-8 aspect-square bg-black text-white opacity-50  rounded-full align-center`}
          >
            {number_possible}
          </div>
          <div
            className={`absolute bottom-0 left-0 w-full  bg-black text-white opacity-50  ${
              corner ? corners[corner] : ""
            }`}
          >
            {collections.find((collection) => collection.id === top_guess)
